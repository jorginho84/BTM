"""
This script computes the ATT of BTM across informational sets (prob of program's knowledge)
...
#Counterfactual choices
n_sim = 50
k_levels = [0, 0.1, 0.3, 0.5, 0.8]
params = []
models = []
modelsSD = []
work = np.zeros((N,n_sim,len(k_levels)))

for j in range(len(k_levels)):
	params.append(parameters.Parameters(gammas_x,married,married,dont_workload,[betas_hat[2], k_levels[j]],delta))
	models.append(util.Utility(params[j], N, ecivil, children, X, psfe, year,educ,age,age_2))
	modelsSD.append(sd.SimData(params[j], N, models[j]))
...
ate_work = np.zeros((N,len(k_levels) - 1))
for j in range(len(k_levels) - 1):
...
#ATTs across PFSE
att = np.zeros((cat_pfse.size - 1,len(k_levels) - 1))
for k in range(len(k_levels) - 1):
...
label_list = []
for j in range(len(k_levels) - 1):
	label_list.append(r'$\kappa$ = ' + '{:04.2f}'.format(k_levels[j+1]))
